...
export default function OfferCardsList(props) {
  const {activeCityOffers, isNearPlaces, onCardHover} = props;

  return (
    <div className={`${isNearPlaces ? 'near-places__list' : 'cities__places-list tabs__content'} places__list`}>
      {activeCityOffers.map((offer) => <OfferCard offer={offer} onCardHover={onCardHover} isNearPlace={isNearPlaces} key={offer.id}/>)}
    </div>
  );
}
